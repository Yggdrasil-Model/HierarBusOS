...
use bus::{TASK_GETPID, TASK_FORK, TASK_EXEC, TASK_WAITPID, TASK_LOADAPP};
use lazy_static::*;
...
impl Busadapter for Task{
    fn handle(&self,body:[usize;3])->isize{
        match body[0]{
            TASK_EXIT_RUNNEXT=>{exit_current_and_run_next(body[1] as i32);1}
            TASK_SUSPEND_RUNNEXT=>{suspend_current_and_run_next();1}
            TASK_EXIT=>{
                println!("[kernel] Application exited with code {}", body[1] as f64);
                exit_current_and_run_next(body[1] as i32);
                panic!("Unreachable in sys_exit!");
            }
            TASK_INIT=>{add_initproc();1}
            TASK_RUNFIRST=>{run_tasks();1}
            TASK_TOKEN=>{current_user_token() as isize}
            TASK_TRAP_CX=>{
                current_trap_cx();
                1
            }
            ...
            TASK_GETPID=>{current_task().unwrap().pid.0 as isize}
            TASK_FORK=>{sys_fork()}
            TASK_EXEC=>{sys_exec(body[1]  as *const u8)}
            TASK_WAITPID=>{sys_waitpid(body[1] as isize, body[2] as *mut i32)}
            TASK_LOADAPP=>{list_apps();1}
